...
function App() {
    ...
    eventSource.onmessage = (event) => {
      try {
        ...
        if (data.type === 'query_generating') {
          ...
          // Handle workflow interruption - show approval modal
          setIsResearching(false);
          setPendingApproval(data);
          setStatus({
            step: "Awaiting Approval",
            message: data.message || "Workflow paused for user approval"
          });
  ...
  // Handle user approval for interrupted workflow
  const handleApprove = async (approvalData: any) => {
    if (!currentJobId) return;

    try {
      const response = await fetch(`${API_URL}/research/${currentJobId}/approve`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      setIsResearching(true);
      setStatus({ step: "Resuming", message: "Continuing research..." });

    } catch (error) {
      console.error('Error approving workflow:', error);
      setError(error instanceof Error ? error.message : 'Failed to approve workflow');
    }
  };
  ...
  const handleReject = async () => {
    ...
    try {
      await fetch(`${API_URL}/research/${currentJobId}/approve`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
